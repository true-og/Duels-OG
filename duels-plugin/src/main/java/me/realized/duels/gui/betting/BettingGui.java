...
public class BettingGui extends AbstractGui<DuelsPlugin> {
    ...
    private final Section[] sections = {new Section(9, 13, 4), new Section(14, 18, 4)};
    ...
    public BettingGui(final DuelsPlugin plugin, final Settings settings, final Player first, final Player second) {
        ...
        this.inventory = InventoryBuilder.of(Component.text(plugin.getLang().getMessage("GUI.item-betting.title")), 54)
                .build();
        ...
        public void run() {
            ...
            if (counter < 5) {
                ...
                if (CompatUtil.isPre1_13()) {
                    item = new ItemStack(item.getType(), item.getAmount(), (short) 5);
                } else {
