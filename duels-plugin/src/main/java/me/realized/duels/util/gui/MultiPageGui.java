...
public class MultiPageGui<P extends JavaPlugin> extends AbstractGui<P> {
    ...
    public MultiPageGui(
            final P plugin, final String title, final int rows, final Collection<? extends Button<P>> buttons) {
        super(plugin);
    ...
    private PageNode createPage(final int page, final int total) {
        return new PageNode(InventoryBuilder.of(Component.text(" (" + page + "/" + total + ")"), size)
