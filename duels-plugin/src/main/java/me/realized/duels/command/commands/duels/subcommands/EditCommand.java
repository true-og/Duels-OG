...
public class EditCommand extends BaseCommand {
    ...
    public EditCommand(final DuelsPlugin plugin) {
        super(
                plugin,
                "edit",
                "edit [name] [add:remove:set] [wins:losses] [amount]",
                "Edits player's wins or losses.",
                5,
                false);
    ...
    protected void execute(final CommandSender sender, final String label, final String[] args) {
        ...
        if (setter == null) {
            lang.sendMessage(
                    sender, "ERROR.command.invalid-option", "option", option, "available_options", getters.keySet());
            return;
        }
