...
public class DuelCommand extends BaseCommand {
    ...
    protected boolean executeFirst(final CommandSender sender, final String label, final String[] args) {
        ...
        if (!Validators.validate(requestManager.getTargetValidators(), player, target, TargetCheckRequestValidator.class)) {
            return true;
        }

        if (requestManager.has(player, target)) {
            lang.sendMessage(sender, "ERROR.duel.already-has-request", "name", args[0]);
            return true;
        }
