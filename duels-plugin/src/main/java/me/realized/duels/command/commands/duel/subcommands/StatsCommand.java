...
public class StatsCommand extends BaseCommand {
    ...
    private void displayStats(final Player sender, final String name) {
        ...
        final String wlRatio = String.valueOf(
                user.getLosses() > 0
                        ? Math.round(((double) user.getWins() / (double) user.getLosses()) * 100.0) / 100.0
                        : user.getWins());
        final String requests = String.valueOf(
                user.canRequest() ? lang.getMessage("GENERAL.enabled") : lang.getMessage("GENERAL.disabled"));
        final Object[] args = {
            "name", user.getName(), "wins", wins, "losses", losses, "wl_ratio", wlRatio, "requests_enabled", requests
        };
        ...
        if (config.isDisplayKitRatings() || config.isDisplayNoKitRating()) {
            ...
            if (config.isDisplayNoKitRating()) {
                lang.sendMessage(
                        sender,
                        "COMMAND.duel.stats.rating.format",
                        "type",
                        config.getTopNoKitType(),
                        "kit",
                        config.getTopNoKitType(),
                        "rating",
                        user.getRating());
        ...
        if (config.isDisplayPastMatches()) {
            ...
            user.getMatches().forEach(match -> {
                ...
                TextBuilder.of(lang.getMessage(
                                "COMMAND.duel.stats.match.format",
                                "winner",
                                match.getWinner(),
                                "loser",
                                match.getLoser()))
                        .setHoverEvent(
                                Action.SHOW_TEXT,
                                lang.getMessage(
                                        "COMMAND.duel.stats.match.hover-text",
                                        "kit",
                                        kit,
                                        "duration",
                                        duration,
